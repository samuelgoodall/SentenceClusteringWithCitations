...
class QualitativeParserBot:
    ...
    def run(self):
        ...
        processes = []
        for i in range(8):
            processes.append(Process(target=self.function_for_process, args=(
                split_of_paper_folder_path_list[i], os.path.join(self.output_folder, "data" + str(i) + ".csv"),
                os.path.join(self.output_folder, "backup" + str(i) + ".txt"))))
        for p in processes:
            p.start()
        for p in processes:
            p.join()
